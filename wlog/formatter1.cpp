...
void formatter1::date(std::ostream& os, const time_point& tp, unsigned long hide, long /*colorized*/)
{
...
void formatter1::time(std::ostream& os,const time_point& tp, unsigned long hide, long )
{
...
void formatter1::ms(std::ostream& os, const time_point& tp, long resolution, unsigned long, long )
{
  ...
  ) const
{
  ...
  */
  formatter1::date(os, tp, 0ul, 0 );
  formatter1::time(os, tp, 0ul, 0);

  if ( _resolution != resolutions::seconds )
  {
    formatter1::ms(os, tp, static_cast<long>(_resolution), 0, 0 );
  }
    //os << "." << mkms();
  os << " " << name << " " << ident << " " << str;
}
